...
@MainActor
private final class BTCChartControls: ObservableObject {
    ...
    func attach(_ chart: InfiniteChartBase, provider: BTCDataProvider) {
        ...
        // Opt-in recording mode follows the same viewport callback and provider
    ...
    private func fitPriceRange() {
        guard let chart, let viewport = chart.viewport,
    ...
    private func scaleVisibleRange(by factor: Double) {
        guard let chart, let range = chart.viewport?.visibleXRange else { return }
    ...
    private func runDemo(_ chart: InfiniteChartBase) async throws {
        ...
        for _ in 0..<20 where chart.viewport == nil {
            try await Task.sleep(nanoseconds: 100_000_000)
        }
        guard let viewport = chart.viewport else { return }
    ...
    private func animate(_ chart: InfiniteChartBase, center: Double, span: Double, duration: Double) async throws {
        guard let range = chart.viewport?.visibleXRange else { return }
    ...
    private func hold(_ chart: InfiniteChartBase, stage: String, duration: Double) async throws {
        ...
        if let range = chart.viewport?.visibleXRange {
...
private func makeBTCChart(provider: BTCDataProvider, controls: BTCChartControls) -> InfiniteChartBase {
    ...
    chart.onViewportChange = { [weak provider, weak controls, weak chart] change in
        guard let chart, let controls, controls.chart === chart else { return }
        provider?.updateViewport(change.viewport)
        controls.updateViewport(change.viewport)
    }
    return chart
}
